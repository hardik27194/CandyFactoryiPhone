//
//  PeanutFinalViewController.m
//  CandyFactory
//
//  Created by Ana-Maria Stoica on 1/4/13.
...
@interface PeanutFinalViewController ()
{
...
@implementation PeanutFinalViewController
...
- (id)init
{
    if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)
        self = [super initWithNibName:@"PeanutFinalViewController-iPad" bundle:[NSBundle mainBundle]];
    else if([[UIScreen mainScreen] bounds].size.height == 568)
        self = [super initWithNibName:@"PeanutFinalViewController-5" bundle:[NSBundle mainBundle]];
    else
        self = [super initWithNibName:@"PeanutFinalViewController" bundle:[NSBundle mainBundle]];
...
{
    self.fullImage.image = backgroundImage1;
    ...
    if([(AppDelegate *)[[UIApplication sharedApplication] delegate] peanutButterPurchased] || [(AppDelegate *)[[UIApplication sharedApplication] delegate] cottonCandyPurchased])
    {
    }
    else
    {
...
- (void)viewDidLoad
{
    [Flurry logEvent:@"Peanut_Butter_Created"];
}
...
- (void)touchesBegan:(NSSet*) touches withEvent:(UIEvent*)event
{
    ...
    if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)
        ...
        newImage = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 320, 568)];
    else
        newImage = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 320, 480)];
    
    newImage.image = self.backImage1;
    ...
    {
        if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)
            biteName = [NSString stringWithFormat:@"biteMask-iPad"];
        else
            biteName = [NSString stringWithFormat:@"biteMask"];
    }
    else
    {
        if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)
            biteName = [NSString stringWithFormat:@"biteMask%d-iPad",biteNumber+1];
        else
            biteName = [NSString stringWithFormat:@"biteMask%d",biteNumber+1];
    }
    biteNumber++;
    ...
    if(actionSheet.tag == 17)
    {
        if(buttonIndex == 0)
        {
            if([SLComposeViewController isAvailableForServiceType:SLServiceTypeFacebook])
            {
                ...
                [mySLComposerSheet addImage:backgroundImage1];
...
- (void)saveToGallery:(NSTimer*)theTimer
{
    ...
    else
    {
        ...
        NSString *nameOfCandy = [NSString stringWithFormat:@"peanut%d",[array count]];
        NSString *CandyBackImage = [NSString stringWithFormat:@"peanutBack%d",[array count]];
        NSString *maskImage2 = [NSString stringWithFormat:@"peanutMask%d",[array count]];
        
        NSMutableDictionary * newCandy = [[NSMutableDictionary alloc]init];
        [newCandy setObject:@"5" forKey:@"type"];
        ...
        NSData *imageData2 = UIImagePNGRepresentation(backgroundImage1);
        ...
        NSData *imageData3 = UIImagePNGRepresentation(backImage1);
...
{
    self.isSaved = YES;
    UIImageWriteToSavedPhotosAlbum(backgroundImage1, nil, nil, nil);
...
- (void)shareToEmail:(NSTimer*)theTimer
{
    if([MFMailComposeViewController canSendMail])
    {
        ...
        NSData *imageData = UIImagePNGRepresentation(backgroundImage1);
